/**
 * DCSA Assignmen3 Color Transfer
 * 4104056004 資工四 李家駿
 * mybmp.h
...
void findmaxRGBvalue(bmpfile,unsigned char*);
void findminRGBvalue(bmpfile,unsigned char*);
void outputbmp(char*,bmpfile,unsigned char*);
...
bmpfile readbmp(char* filename)
{
    ...
    findmaxRGBvalue(bmp_f,max);                                 //Get max RGB value
    findminRGBvalue(bmp_f,min);                                 //GET min RGB value
...
bmpfile bmp_colortransfer(bmpfile src_bmp,bmpfile tar_bmp)
{
    ...
    for(i=0;i<src_bmp.height;i++)
    {
        for(j=0;j<src_bmp.width;j++)
        {
            ...
            /*If the value is not in [0,255],makes value under 0 equal to min RGB value of target file.
                .Make those above 255 equal to max RGB value of target.*/
            /*if(R>255) R=tar_bmp.max[0]; 
            if(G>255) G=tar_bmp.max[1];
            if(B>255) B=tar_bmp.max[2];
            if(R<0) R=tar_bmp.min[0];
            if(G<0) G=tar_bmp.min[1];
            if(B<0) B=tar_bmp.min[2];*/
...
void findmaxRGBvalue(bmpfile bmp_f,unsigned char max[3])
{
    int i,j,max_index,R=1,G=0,B=2;
    max_index=R;
    for(i=R;i<bmp_f.bitmap_datasize;i+=3)
        if(bmp_f.image_raw[max_index]<bmp_f.image_raw[i])
            max_index=i;
    max[0]=bmp_f.image_raw[max_index];
    max_index=G;
    for(i=G;i<bmp_f.bitmap_datasize;i+=3)
        if(bmp_f.image_raw[max_index]<bmp_f.image_raw[i])
            max_index=i;
    max[1]=bmp_f.image_raw[max_index];
    max_index=B;
    for(i=B;i<bmp_f.bitmap_datasize;i+=3)
        if(bmp_f.image_raw[max_index]<bmp_f.image_raw[i])
            max_index=i;
    max[2]=bmp_f.image_raw[max_index];
}
void findminRGBvalue(bmpfile bmp_f,unsigned char min[3])
{
    int i,j,min_index,R=1,G=0,B=2;
    min_index=R;
    for(i=R;i<bmp_f.bitmap_datasize;i+=3)
        if(bmp_f.image_raw[min_index]>bmp_f.image_raw[i])
            min_index=i;
    min[0]=bmp_f.image_raw[min_index];
    min_index=G;
    for(i=G;i<bmp_f.bitmap_datasize;i+=3)
        if(bmp_f.image_raw[min_index]>bmp_f.image_raw[i])
            min_index=i;
    min[1]=bmp_f.image_raw[min_index];
    min_index=B;
    for(i=B;i<bmp_f.bitmap_datasize;i+=3)
        if(bmp_f.image_raw[min_index]>bmp_f.image_raw[i])
            min_index=i;
    min[2]=bmp_f.image_raw[min_index];
}
void outputbmp(char*filename,bmpfile bmp_f,unsigned char* image_raw)
{
