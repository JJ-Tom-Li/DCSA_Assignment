/**
 * DCSA Assignmen1_Improving
 * 4104056004 資工四 李家駿
 * Usage: ./4104056004-李家駿-DCSA-01_I [DIRECTORY_NAME]
*/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
int main(int argc, char *argv[])
{
    ...
    while((ptr=readdir(bmp_dir))!=NULL)
    {
        if(strlen(ptr->d_name)<=4 || is_bmp(ptr->d_name)!=0 ) continue; //無效檔案
        strcpy(bmp_filename,argv[1]);
        strcat(bmp_filename,"/");
        strcat(bmp_filename,ptr->d_name);
        c=readbmp(bmp_filename);
        if(c==-1)
            printf("Error.\n");
    }
